...
class Quantize(nn.Module):
    def __init__(self, dim, n_embed, decay=0.99, eps=1e-5):
        ...
        # self.embed = embed
        # self.cluster_size = torch.zeros(n_embed)
...
class RVQ(nn.Module):
    def __init__(self,dim, n_embed, decay=0.99, eps=1e-5,n_quantizers=1,):
        super().__init__()

        self.n_quantizers = n_quantizers
        self.quantizers = [Quantize(dim,n_embed,decay,eps).to('cuda') for _ in range(self.n_quantizers)]
    ...
    def forward(self,x):
        ...
        for quantizer in self.quantizers:
            quantize, diff, embed_ind = quantizer(x)
            quantize_all.append(quantize)
            diff_all.append(diff)
            embed_ind_all.append(embed_ind)
        quantize_all = sum(quantize_all)
        diff_all = sum(diff_all) / self.n_quantizers
        embed_ind_all = torch.stack(embed_ind_all,dim=0)
        return quantize_all,diff_all,embed_ind_all

    def codebooks(self):
        return torch.stack([quantizer.embed for quantizer in self.quantizers],dim=0)

    def embed_code(self,embed_ind_all):
        return torch.sum([quantizer.embed_code(embed_ind) for embed_ind,quantizer in zip(embed_ind_all,self.quantizers)])
    ...
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 4:
            ...
            blocks_2 = [
                ...
                nn.Conv2d(channel // 2, channel, 2, stride=2, padding=0),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel, channel , 2, stride=2, padding=0),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel , channel, 3, padding=1),
            ]
            ...
            blocks_3 = [
                ...
                nn.Conv2d(channel // 2, channel, 6, stride=2, padding=2),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel, channel , 6, stride=2, padding=2),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel , channel, 3, padding=1),
            ]
            ...
            blocks_4 = [
                ...
                nn.Conv2d(channel // 2, channel, 8, stride=2, padding=3),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel, channel , 8, stride=2, padding=3),
                nn.ReLU(inplace=True),
                nn.Conv2d(channel , channel, 3, padding=1),
            ]
...
class RVQVAE(nn.Module):
    def __init__(
        ...
        # self.dec_t = Decoder(embed_dim, embed_dim, channel, n_res_block, n_res_channel, stride=2)
        self.quantize_conv_b = nn.Conv2d(channel, embed_dim, 1)
        self.quantize_b = RVQ(embed_dim, n_embed,n_quantizers=n_quantizers)
        # self.upsample_t = nn.ConvTranspose2d(
        #     embed_dim, embed_dim, 4, stride=2, padding=1
        # )
        self.dec = Decoder(
