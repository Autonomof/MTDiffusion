...
def get_dataset_filelist(label=None) -> list:
    print(label)
    training_files: List[dict] = list()
    for root_dir, _, file_list in os.walk("./DCASEFoleySoundSynthesisDevSet"):
        for file_name in file_list:
            if label:
                if os.path.splitext(file_name)[-1] == ".wav" and label in root_dir:
                    # print(clas_dict[root_dir.split("/")[-1]],f"{root_dir}/{file_name}")
