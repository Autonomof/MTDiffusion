...
from loss import WeightedCrossEntropy, BinaryCrossEntropy
from metrics import metrics, metrics_tagging
...
def train_inception_scorer(cfg):
    ...
    train_ids_path = '/mnt/autonomf_4T/text-to-sound-Synthesis-master/Codebook/specvqgan/modules/losses/vggishish/data/audioset_train.txt'
    cache_path = '/mnt/autonomf_4T/text-to-sound-Synthesis-master/Codebook/specvqgan/modules/losses/vggishish/data/'
    ...
    datasets = {
        'train': Audioset('train', cfg.mels_path, transforms),
        'valid': Audioset('valid', cfg.mels_path, transforms),
    ...
    model = Melception(num_classes=527)
    ...
    criterion = BinaryCrossEntropy(smoothing=0.1, reduction='mean')
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for i, batch in enumerate(prog_bar):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        # print('aux_outputs ', aux_outputs.shape)
                        # assert 1==2
                        loss1 = criterion(outputs, targets)
                        loss2 = criterion(aux_outputs, targets)
                        loss = loss1 + 0.4*loss2
                        # loss = criterion(outputs, targets) # ? check the melception loss
                    else:
            ...
            metrics_dict = metrics_tagging(targets_from_each_batch, preds_from_each_batch)
    ...
    preds_from_each_batch = torch.cat(preds_from_each_batch)
    targets_from_each_batch = torch.cat(targets_from_each_batch)
    test_metrics_dict = metrics_tagging(targets_from_each_batch, preds_from_each_batch)
...
if __name__ == '__main__':
    ...
    cfg_yml = OmegaConf.load('/mnt/autonomf_4T/text-to-sound-Synthesis-master/Codebook/specvqgan/modules/losses/vggishish/configs/melception.yaml')
    # the latter arguments are prioritized
    cfg = cfg_yml
    OmegaConf.set_readonly(cfg, True)
    # print(OmegaConf.to_yaml(cfg))
