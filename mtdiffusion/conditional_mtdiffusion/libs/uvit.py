...
def unpatchify(x, channels=3, img_size=None):
    if img_size == None:
        ...
        # print(h,w,list(x.shape),patch_size,channels)
        assert h * w == x.shape[1] and patch_size ** 2 * channels == x.shape[2]
        x = einops.rearrange(x, 'B (h w) (p1 p2 C) -> B C (h p1) (w p2)', h=h, p1=patch_size, p2=patch_size)
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, time_embed_dim = None, mlp_ratio=4., qkv_bias=False, qk_scale=None,
    ...
    def forward(self, x, time, skip=None):
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(self._forward, x, skip)
        else:
            return self._forward(x, time, skip)

    def _forward(self, x, time, skip=None):
        if self.skip_linear is not None:
            x = self.skip_linear(torch.cat([x, skip], dim=-1))
        time_embed = self.time_mlp(time)
        a, b = time_embed.chunk(2,dim=-1)
        x = x * (1+a) + b
...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=1, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.in_blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, time_embed_dim=embed_dim, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x, timesteps, y=None):
        ...
        x = self.mid_block(x, time_token)

        for blk in self.out_blocks:
            x = blk(x, time_token, skips.pop())

        x = self.norm(x)
        x = self.decoder_pred(x)
        # print(list(x.shape),self.extras,L,'do')
        assert x.size(1) == self.extras + L
        x = x[:, self.extras:, :]
        x = unpatchify(x, self.in_chans, self.img_size)
