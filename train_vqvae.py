...
from vqvae import RVQVAE
from scheduler import CycleScheduler

import audio2mel
from datasets import get_dataset_filelist

os.environ['CUDA_VISIBEL_DEVICES'] = '1'
...
def main(args):
    ...
    n_quantizers = 4
    model = RVQVAE(n_quantizers=n_quantizers)
    ...
    start_point = 0
    ckpt = 'checkpoint/vqvae/vqvae_076.pt'
    # ckpt = None
    if ckpt is not None:
        _, start_point = ckpt.split('_')
        start_point = int(start_point[0:-3])
        model.load_state_dict(torch.load(ckpt))
    model = model.to(device)
    for i in range(start_point, args.epoch):
