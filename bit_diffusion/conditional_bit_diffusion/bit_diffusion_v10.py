...
class LinearAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64,addtion_dim=None):
        super().__init__()
        ...
        self.addtion_dim = addtion_dim
        if addtion_dim != None:
            self.mlp_addtion = nn.Sequential(
            nn.SiLU(),
            nn.Linear(addtion_dim,hidden_dim)
        )

    def forward(self, x, addtion=None):
        b, c, h, w = x.shape
        qkv = self.to_qkv(x).chunk(3, dim=1)

        if self.addtion_dim!=None and addtion != None:
            addtion = self.mlp_addtion(addtion)
            addtion = rearrange(addtion, 'b c -> b c 1 1')
            qkv = qkv[0]+addtion,qkv[1],qkv[2]
...
class Unet(nn.Module):
    def __init__(
        ...
        # layers
        self.attention_control_dim = 128
        self.attention_control = torch.nn.Parameter(torch.zeros(self.attention_control_dim))
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim=time_dim),
                PreNorm(dim_in, LinearAttention(dim_in,addtion_dim=time_dim)),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=1)
            ]))
        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim,addtion_dim=time_dim)))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
                Residual(PreNorm(dim_out, LinearAttention(dim_out,addtion_dim=time_dim))),
