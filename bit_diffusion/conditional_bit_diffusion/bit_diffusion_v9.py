...
class BitDiffusion(nn.Module):
    ...
    def mix_image(self,imgs,labels):
        batch = imgs.shape[0]
        for i in range(batch):
            seed = random.randint(0,batch-1)
            if labels[seed]==labels[i] and random.random() < 0.5:continue
            else:
                imgs[i]=imgs[i]+imgs[seed]
        return imgs
    ...
    def forward(self, img, labels, *args, **kwargs):
        ...
        mix_img = self.mix_image(img,labels)
        ...
        noised_img = alpha * mix_img + sigma * noise
