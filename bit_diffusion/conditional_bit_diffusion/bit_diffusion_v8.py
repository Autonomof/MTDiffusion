...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *,label_emb_dim=None, time_emb_dim=None, groups=16):
        super().__init__()
        ...
        ) if exists(label_emb_dim) else None

        self.block1 = Block(dim, dim_out, groups=groups)
        self.block2 = Block(dim_out, dim_out * 2, groups=groups)
        self.res_conv = nn.Conv2d(dim, dim_out, 1) if dim != dim_out else nn.Identity()

        self.dropout = torch.nn.Dropout(p=0.1)

    def forward(self, x, label_emb=None,time_emb=None):
        ...
        if exists(self.mlp_label) and exists(label_emb):
            ...
            label_emb = rearrange(label_emb, 'b c -> b c 1 1')
            scale_shift = label_emb.chunk(2, dim=1)
...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                block_klass(dim_in, dim_in, time_emb_dim=time_dim,label_emb_dim=time_dim),
                block_klass(dim_in, dim_in, time_emb_dim=time_dim,label_emb_dim=time_dim),
                Residual(PreNorm(dim_in, LinearAttention(dim_in))),
                Downsample(dim_in, dim_out) if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=1)
            ]))
        mid_dim = dims[-1]
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim,label_emb_dim=time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim,label_emb_dim=time_dim)

        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            is_last = ind == (len(in_out) - 1)

            self.ups.append(nn.ModuleList([
                block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim,label_emb_dim=time_dim),
                block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim,label_emb_dim=time_dim),
                Residual(PreNorm(dim_out, LinearAttention(dim_out))),
                Upsample(dim_out, dim_in) if not is_last else nn.Conv2d(dim_out, dim_in, 3, padding=1)
            ]))

        self.final_res_block = block_klass(dim * 2, dim, time_emb_dim=time_dim,label_emb_dim=time_dim)
        ...
        self.final_res_block_noise = block_klass(dim * 2, dim, time_emb_dim=time_dim,label_emb_dim=time_dim)
    ...
    def forward(self, x, time, label):
        ...
        if label != None:
            label = self.label_embedding(label).squeeze(1)
            # t += c.squeeze(1)
        ...
        x = self.mid_block1(x, label, t)
        x = self.mid_attn(x)
        x = self.mid_block2(x, label, t)
        ...
        x = self.final_res_block(x, label, t)

        for block1, block2, attn, upsample in self.ups:
            x_noise = torch.cat((x_noise, h_noise.pop()), dim=1)
            x_noise = block1(x_noise, label, t)

            x_noise = torch.cat((x_noise, h_noise.pop()), dim=1)
            x_noise = block2(x_noise, label, t)
        ...
        x_noise = self.final_res_block(x_noise, label, t)
...
class BitDiffusion(nn.Module):
    def __init__(
        ...
        # self.scale_upsample = ScaleUpsample(dim=BITS)
        # self.scale_downsample = ScaleDownsample(dim=BITS)
        # for p in self.scale_downsample.parameters():
        #     p.requires_grad = False
        # for p in self.scale_upsample.parameters():
        #     p.requires_grad = False
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, labels, cfg_scale=3, time_difference=None):
        batch, device = shape[0], self.device
        ...
        for times, times_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            x_start,_ = self.model(img, log_snr, labels)
            if cfg_scale > 0:
                x_start_uncond, pred_noise_uncond = self.model(img, log_snr, labels)
                x_start = torch.lerp(x_start_uncond, x_start, cfg_scale)
    ...
    @torch.no_grad()
    def sample(self, labels, batch_size=16, cfg_scale=3):
        image_size, channels = self.image_size, self.channels
        sample_fn = self.ddpm_sample if not self.use_ddim else self.ddim_sample
        return sample_fn((batch_size, channels, image_size[0], image_size[1]), labels, cfg_scale=cfg_scale)
    ...
    def forward(self, img, labels, *args, **kwargs):
        ...
        # print(img_size,img.shape,raw_img.shape)
        # img = self.scale_upsample(raw_img)
        ...
        pred_noise = (noised_img - alpha * pred_img) / sigma.clamp(min=1e-8)
    ...
    #
    # def scale_downsample(self,x):
    #     return self.scale_downsample(x)
